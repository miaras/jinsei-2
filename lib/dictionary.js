...
import { normalize as normalizeKorean, tokenize as tokenizeKorean } from 'oktjs';
...
function koreanCandidates(input) {
  ...
  // oktjs ports Open Korean Text directly to JavaScript. Predicate tokens
  // expose their dictionary-form stem, including irregular conjugations.
  try {
    const tokens = tokenizeKorean(normalizeKorean(term));
    tokens.forEach((token, index) => {
      if ((token.pos !== 'Verb' && token.pos !== 'Adjective') || !token.stem) return;
      const nounPrefix = tokens.slice(0, index).filter(item => item.pos === 'Noun').map(item => item.text).join('');
      if (nounPrefix && token.stem === '하다') add(`${nounPrefix}하다`);
      add(token.stem);
    });
    tokens.filter(token => token.pos === 'Noun').forEach(token => add(token.text));
  } catch (error) {
    // Retain the lightweight rules below as a deployment-safe fallback.
  }

  // Fallbacks also handle mixed Hanja/Hangul tokens that oktjs may not tag.
